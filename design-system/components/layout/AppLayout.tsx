...
import { Button } from "./primitives/Button.tsx" assert { type: "tsx" };
import { Avatar } from "./primitives/Display.tsx" assert { type: "tsx" };
import { SimpleTooltip } from "./primitives/Display.tsx" assert { type: "tsx" };
import { Divider } from "./primitives/Display.tsx" assert { type: "tsx" };
...
interface AppLayoutProps {
  ...
  topbarSlot?: React.ReactNode;  // custom content for topbar right side
    ...
    <SidebarStateContext.Provider value={{ collapsed, toggle, mobileOpen, setMobileOpen }}>
      <div className="flex h-svh overflow-hidden bg-background">
        ...
        <div className="flex flex-1 flex-col min-w-0 overflow-hidden">
          {upgradeSlot}
          <AppTopbar topbarSlot={topbarSlot} />
          <main
            id="main-content"
...
function AppSidebar({
  ...
  logo, collapsed, onToggle, className,
}: AppSidebarProps) {
  // Group nav items
  ...
  return (
    ...
    >
      ...
      <div className={cn(
        ...
        {!collapsed && (
          <span className="font-semibold text-sm text-foreground truncate">MedFlow</span>
        )}
      </div>
      ...
      <div className={cn(
        ...
        {!collapsed ? (
          <div className="flex items-center gap-2.5 px-2 py-1.5 rounded-lg hover:bg-sidebar-accent transition-colors">
            ...
            <SimpleTooltip content="Log out" side="right">
              <button
                onClick={onLogout}
                aria-label="Log out"
          ...
          <SimpleTooltip content={`${user.name} — Log out`} side="right">
            <button
              onClick={onLogout}
              aria-label={`Log out as ${user.name}`}
              className="w-full flex justify-center p-1.5 rounded-lg hover:bg-sidebar-accent transition-colors"
            >
        ...
        >
          {collapsed
            ...
            : <><PanelLeftClose className="h-3.5 w-3.5" aria-hidden /><span>Collapse</span></>
          }
        </button>
      </div>
    </aside>
...
function AppTopbar({ topbarSlot }: AppTopbarProps) {
